...
const TeacherDashboard: React.FC = () => {
  const { getClassesCount, students } = useStudentsStore()
  const { getTodayAttendanceCount } = useAttendanceStore()

  const myClasses = getClassesCount() // demo: toàn bộ lớp coi như lớp phụ trách
  const todaySessions = 1 // demo: giả lập 1 buổi hôm nay
  const absentCount = students.filter((s) => s.status === 'absent').length
  const lateCount = 0 // demo: chưa có dữ liệu muộn
        ...
        <div className={styles.chartsSection}>
          <div className={styles.chartCard}>
            ...
            <div className={styles.chartPlaceholder}>
              ...
              <p>Tổng lượt điểm danh: {getTodayAttendanceCount()}</p>
              <span>Chi tiết sẽ hiển thị tại mục Điểm danh</span>
            </div>
          </div>
          ...
          <div className={styles.chartCard}>
            ...
            <div className={styles.chartPlaceholder}>
              ...
              <p>Vắng: {absentCount} • Muộn: {lateCount}</p>
              <span>Sẽ nối dữ liệu muộn khi có</span>
            </div>
          </div>
        </div>
      </div>
