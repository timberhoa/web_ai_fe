...
import SessionCreatedModal from '../../../components/SessionCreatedModal/SessionCreatedModal'
...
const Schedule: React.FC = () => {
  ...
  // Session creation notification states
  const [showCreatedNotification, setShowCreatedNotification] = useState(false)
  const [createdSessions, setCreatedSessions] = useState<ClassSession | ClassSession[] | null>(null)
  const [isRecurringCreated, setIsRecurringCreated] = useState(false)
  ...
  const handleCreateSingle = async (event: React.FormEvent) => {
    ...
    try {
      const createdSession = await scheduleApi.create(buildSinglePayload(singleForm))
      setSingleForm(defaultSingleForm)
      setFeedback('Session created')

      // Show notification modal
      setCreatedSessions(createdSession)
      setIsRecurringCreated(false)
  ...
  const handleCreateRecurring = async (event: React.FormEvent) => {
    ...
    try {
      const createdSessions = await scheduleApi.createRecurring(buildRecurringPayload(recurringForm))
      setRecurringForm(defaultRecurringForm)
      setFeedback('Recurring sessions created')

      // Show notification modal
      setCreatedSessions(createdSessions)
      setIsRecurringCreated(true)
      setShowCreatedNotification(true)
  ...
  return (
    <div className={`${adminStyles.page} ${styles.root}`}>
              ...
              {!sessionsLoading &&
                sessionRows.map((session) => (
                  <tr key={session.sessionId}>
                    ...
                    <td data-label="Giảng viên">{session.teacherName || '-'}</td>
                    <td data-label="Bắt đầu">{session.startLabel}</td>
                    <td data-label="Kết thúc">{session.endLabel}</td>
                    <td data-label="Phòng">{session.roomName}</td>
                    <td data-label="Trạng thái">
                      <span className={session.locked ? styles.locked : styles.unlocked}>
                        {session.locked ? 'Locked' : 'Open'}
                      </span>
                    </td>
                    <td className={styles.actionsCell} data-label="Hành động">
      ...
      {/* Session Created Notification Modal */}
      {createdSessions && (
        <SessionCreatedModal
          isOpen={showCreatedNotification}
          onClose={() => {
            setShowCreatedNotification(false)
            setCreatedSessions(null)
          }}
          sessions={createdSessions}
          isRecurring={isRecurringCreated}
        />
      )}
    </div>
  )
